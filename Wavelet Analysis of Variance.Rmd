...
Lastly, and just for good measure, I'll calculate a basic metric for the extent of inflation per item during the pandemic.  The following gives the percentage change in price for each item between January 2020 and the most recent month (currently Nov. 2021). A histogram is plotted, because who doesn't like a histogram, though I'm not presently comparing it to other periods. Also shown are the highest 10 items.
...
The items that have been making headlines are clear here, most notably the used car market. And within that the used auto margin (the difference between what dealers pay for used cars and what they charge).  Used light trucks doesn't have margin data for this period (though we could--and should--run the same for 1983 on to get it), but I'm guessing it's the dealer margin there, too. Meats are also showing up, especially beef; but so are major household appliances.
