...
```{r write csv for high inflation items, eval = F}

exportP <- data.frame("Date" = rownames(priSeries$item[i]))
exportQ <- data.frame("Date" = rownames(priSeries$item[i]))

for(i in 1:nrow(hiInfItems)){
  # find index of item
  loc <- which(priSeries$name == hiInfItems$Item[i])
  exportP <- cbind(exportP, priSeries$item[loc])
  colnames(exportP)[i+1] <- hiInfItems$Item[i]
  exportQ <- cbind(exportQ, quaSeries$item[loc])
  colnames(exportQ)[i+1] <- hiInfItems$Item[i]
...
write.csv(exportP, "hiInfPrices.csv")
write.csv(exportQ, "hiInfquantities.csv")
