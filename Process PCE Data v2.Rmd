...
It also has chunks for selecting certain items that we're looking at in our research.
...
Below is some sample code for plotting a selection from the table.
...
# plot the series
plot(series, main = priSeries$name[i], ylab = "Price")
...
Select high inflation items for analysis.

```{r save tables to csv}

# get the table from the function
priSeries <- makeTimeSeries(pri, startDate = "1983-01-01")
quaSeries <- makeTimeSeries(qua, startDate = "1983-01-01")
expSeries <- makeTimeSeries(exp, startDate = "1983-01-01")
...
priS <- p[, c(1:5, 7, 8, 47, 48, 50, 84, 85, 89, 90)]
quaS <- q[, c(1:5, 7, 8, 47, 48, 50, 84, 85, 89, 90)]
expS <- e[, c(1:5, 7, 8, 47, 48, 50, 84, 85, 89, 90)]

write.csv(priS, "pricesSelect.csv")
write.csv(quaS, "quantitiesSelect.csv")
write.csv(expS, "expendituresSelect.csv")
